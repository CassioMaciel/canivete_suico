...
def test_fatorial_type_error():
    ...
    assert (
        str(excinfo.value) == 'A entrada desse programa só pode ser do '
        'tipo int'
    )
...
def test_baskara_4():
    ...
    assert (
        str(excinfo.value) == 'A equação 2.0x²+2.0x+2.0=0 não tem valor '
        'real'
    )
...
def test_baskara_5():
    ...
    assert (
        str(excinfo.value) == 'Todas as entradas desse programa devem '
        'ser um número'
    )
...
def test_bissexto_1():
    ...
    assert (
        str(excinfo.value) == 'O ano deve ser do tipo datetime.date ou '
...
@mark.parametrize(
    ...
    [
        (2000, True),
        (datetime.date(year=2000, month=1, day=1), True),
        (2003, False),
        (2003, False),
        (datetime.date(year=2003, month=1, day=1), False),
        (2022, False),
        (datetime.date(year=2022, month=1, day=1), False),
        (2023, False),
        (datetime.date(year=2023, month=1, day=1), False),
        (2024, True),
        (datetime.date(year=2024, month=1, day=1), True),
        (2100, False),
        (datetime.date(year=2100, month=1, day=1), False),
    ],
)
